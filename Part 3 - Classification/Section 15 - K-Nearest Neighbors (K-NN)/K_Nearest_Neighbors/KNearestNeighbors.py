...
"""
Created on Wed Feb 28 01:40:24 2018
...
classifier=KNeighborsClassifier(n_neighbors=5,metric='minkowski',p=2) #to have euclidian distance
...
plt.contourf(X1, X2, classifier.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
             alpha = 0.75, cmap = ListedColormap(('brown', 'green'))) #Colors the map and draws the line
plt.xlim(X1.min(), X1.max())
plt.ylim(X2.min(), X2.max())
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                c = ListedColormap(('brown', 'green'))(i), label = j) #Colors the points of the set
